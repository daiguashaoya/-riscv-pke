...
// added @ lab4_challenge2 for vfs-based ELF loading
typedef struct elf_info_vfs_t {
  struct file *f;
  process *p;
} elf_info_vfs;

static uint64 elf_fpread_vfs(elf_ctx *ctx, void *dest, uint64 nb,
                             uint64 offset) {
...
static elf_status elf_load_vfs(elf_ctx *ctx) {
  ...
  for (i = 0, off = ctx->ehdr.phoff; i < ctx->ehdr.phnum;
       i++, off += sizeof(ph_addr)) {
    if (elf_fpread_vfs(ctx, (void *)&ph_addr, sizeof(ph_addr), off) !=
        sizeof(ph_addr))
      return EL_EIO;
    ...
    ((process *)(((elf_info_vfs *)(ctx->info))->p))->mapped_info[j].va =
        ph_addr.vaddr;
    ((process *)(((elf_info_vfs *)(ctx->info))->p))->mapped_info[j].npages = 1;

    if (ph_addr.flags == (SEGMENT_READABLE | SEGMENT_EXECUTABLE)) {
      ((process *)(((elf_info_vfs *)(ctx->info))->p))->mapped_info[j].seg_type =
          CODE_SEGMENT;
      sprint("CODE_SEGMENT added at mapped info offset:%d\n", j);
    } else if (ph_addr.flags == (SEGMENT_READABLE | SEGMENT_WRITABLE)) {
      ((process *)(((elf_info_vfs *)(ctx->info))->p))->mapped_info[j].seg_type =
