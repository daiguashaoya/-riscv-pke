...
// loading. this allocater is heavily modified @lab2_1, where we do NOT work in
// bare mode.
//
...
// and store the strings in arg_bug_msg.
//
...
// Each hart loads its own application: hart 0 -> argv[0], hart 1 -> argv[1],
// etc. added multi-hart support @lab2_challenge3
//
void load_bincode_from_host_elf(process *p) {
  ...
  // Read hartid from tp register to select the correct app. added
  // @lab2_challenge3
  uint64 hartid;
  asm volatile("mv %0, tp" : "=r"(hartid));

  // Guard: make sure there is an app for this hart
  if (hartid >= argc)
    panic("Not enough application arguments for hart %d!\n", hartid);
  ...
  sprint(
      "hartid = %d: Application program entry point (virtual address): 0x%lx\n",
