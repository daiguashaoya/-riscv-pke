...
void m_start(uintptr_t hartid, uintptr_t dtb) {
  // Only hart0 initializes the spike file interface and HTIF/memory.
  // Other harts must wait for hart0 to finish before accessing these resources.
  if (hartid == 0) {
    spike_file_init();
    init_dtb(dtb);
  }
  ...
  // init timing (each hart has its own independent timer). added @lab1_3
  timerinit(hartid);
