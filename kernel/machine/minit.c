...
//
// NCPU is defined in kernel/config.h (set to 2 for challenge3).
//
...
// g_itrframe stores registers when interrupt happens in M mode. added @lab1_2
...
void m_start(uintptr_t hartid, uintptr_t dtb) {
  if (hartid == 0) {
    // Only hart 0 initializes the spike file interface and HTIF/memory.
    spike_file_init();
