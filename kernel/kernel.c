...
void load_user_program(process *proc) {
  ...
  // 初始化堆（USER_FREE_ADDRESS_START = 0x400000，即 4MB 处）
  proc->heap_bottom = USER_FREE_ADDRESS_START;
  proc->heap_top = USER_FREE_ADDRESS_START;
  proc->heap_head = NULL;
  proc->heap_pages_cnt = 0;
}
...
int s_start(void) {
  ...
  enable_paging();
  // the code now formally works in paging mode, meaning the page table is now
  // in use.
