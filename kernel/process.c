...
int do_fork(process *parent) {
  ...
  for (int i = 0; i < parent->total_mapped_region; i++) {
    ...
    //     PGSIZE); user_vm_map((pagetable_t)child->pagetable, heap_block,
...
// added @lab4_challenge3 for wait
int do_wait(int pid) {
  for (int i = 0; i < NPROC; i++) {
    process *p = &procs[i];
    if (p->status != FREE && p->pid == pid && p->parent == current) {
      // 如果子进程还没有变成 ZOMBIE，则一直阻塞（Yield）
      while (p->status != ZOMBIE && p->status != FREE) {
        current->status = BLOCKED;
        schedule();
      }
      p->status = FREE; // 回收进程结构
      return 0;
    }
  }
  return -1; // 没有找到对应的子进程
...
// added @lab4_challenge3 for exec
int do_exec(char *path, char *para) {
  // 1. 清除当前进程原有的一些内存映射信息 (简单处理可以直接清空 CODE 和 DATA
  // 段)
  for (int i = 0; i < current->total_mapped_region; i++) {
    if (current->mapped_info[i].seg_type == CODE_SEGMENT ||
      ...
      current->mapped_info[i].va =
          0; // 置 0 代表腾出槽位，物理内存为了简单这里暂不回收
    }
  }

  // 2. 加载新的 ELF 可执行文件，该函数会更新 epc (下一条指令地址)
  // 注意：需要确保加载成功，内部可能用到 vfs_open，如果失败会 panic。
  load_bincode_from_host_elf(current, path);

  // 3. 构建 main 传参：将 path 和 para 存入用户栈，并构造 argv 数组
  uint64 sp = USER_STACK_TOP;

  // 将 argument string 的内容压入用户栈，注意需要在物理地址处写入
  sp -= strlen(para) + 1;
  strcpy((char *)lookup_pa(current->pagetable, sp), para);
  uint64 para_addr = sp;

  sp -= strlen(path) + 1;
  strcpy((char *)lookup_pa(current->pagetable, sp), path);
  uint64 path_addr = sp;
  ...
  // argv[2] = NULL
  sp -= 8;
  *(uint64 *)lookup_pa(current->pagetable, sp) = 0;

  // argv[1] = para_addr
  sp -= 8;
  *(uint64 *)lookup_pa(current->pagetable, sp) = para_addr;

  // argv[0] = path_addr
  sp -= 8;
  *(uint64 *)lookup_pa(current->pagetable, sp) = path_addr;

  // 4. 设置陷阱帧里的参数与栈指针，满足 RISC-V C 调用约定
  current->trapframe->regs.a0 = 2;  // argc
  current->trapframe->regs.a1 = sp; // argv
  current->trapframe->regs.sp = sp; // rsp

  return 0; // exec 通常只有失败才有返回，此处为兼容处理返回 0 会在 Trap
            // 结束后跳转到新 ELF
}
